import{i as A,t as e,a as ge}from"./i18n-cfzYsXIq.js";import{o as ye,h as oe,g as y,e as v,b as S,q as x,k as B,c as E,j as D,p as X,u as L,E as le,r as re,t as ce,a as Z,f as ee}from"./firebase-8Bk_msJm.js";A();const C=[{id:1,name:"Endurance Base Building",season:"Winter 2026",daysPerWeek:4,priority:"High",progress:72,tasks:"18 / 25 workouts completed",due:"Feb 28, 2026",status:"In Progress"},{id:2,name:"Sprint Technique Focus",season:"Spring 2026",daysPerWeek:3,priority:"Medium",progress:45,tasks:"9 / 20 workouts completed",due:"Mar 15, 2026",status:"In Progress"},{id:3,name:"Stroke Refinement (Butterfly)",season:"Summer 2026",daysPerWeek:5,priority:"Low",progress:0,tasks:"0 / 12 workouts completed",due:"Apr 30, 2026",status:"Not Started"},{id:4,name:"Fall Conditioning",season:"Fall 2025",daysPerWeek:3,priority:"High",progress:100,tasks:"30 / 30 workouts completed",due:"Nov 20, 2025",status:"Completed"}];let w=[],N=[],p=null,g="swimmer",b=null,o=null,I=null,k=[],m="overview",se=!1;function we(){const s=document.getElementById("app");s.innerHTML=`
  ...
  `,console.log("Dashboard: Initializing auth listener...");const a=setTimeout(()=>{console.warn("Dashboard: Auth listener timed out — redirecting to signin"),window.location.href="/dragonwebsite/signin.html"},5e3);ye(S,async n=>{if(clearTimeout(a),!n){console.log("Dashboard: No user authenticated, redirecting to signin..."),window.location.href="/dragonwebsite/signin.html";return}p=n,console.log("Dashboard: User authenticated:",n.email);try{console.log("Dashboard: Fetching user document...");const t=await oe(y(v,"users",n.uid));b=t.exists()?t.data().role:null;const i=n.email&&n.email.toLowerCase()==="dragonswim@outlook.com";g=b==="coach"||b==="admin"||i?"coach":b||"swimmer",console.log("Dashboard: Detected role:",g),se?(console.log("Dashboard: Refreshing UI..."),_()):(console.log("Dashboard: Initializing data listeners..."),$e(),se=!0,_())}catch(t){console.error("Dashboard Critical Error:",t),s.innerHTML=`
      ...
      `,g="swimmer"}})}function $e(){const s=x(E(v,"meets"),B("createdAt","desc"));D(s,n=>{w=n.docs.map(t=>({id:t.id,...t.data()})),_()},n=>{console.error("Error listening to meets:",n)});const a=x(E(v,"schedules"),B("createdAt","asc"));if(D(a,n=>{N=n.docs.map(t=>({id:t.id,...t.data()})),_()},n=>{console.error("Error listening to schedules:",n)}),me(),g==="coach"){const n=x(E(v,"registrations"),B("createdAt","desc"));D(n,t=>{k=t.docs.map(i=>({id:i.id,...i.data()})),_()},t=>{console.error("Error listening to registrations:",t)})}}async function me(){if(!p)return;const s=y(v,"registrations",p.uid),a=await oe(s);a.exists()&&(I=a.id,o=a.data())}function _(){p&&me().then(()=>{ae()}).catch(s=>{console.error("Error fetching family data:",s),ae()})}function ae(){g==="coach"?ke(p):Ie(p)}const Ee=["dash_day_sunday","dash_day_monday","dash_day_tuesday","dash_day_wednesday","dash_day_thursday","dash_day_friday","dash_day_saturday"];function $(s){return e(Ee[s]||"dash_day_monday")}function Ie(s){const a=document.getElementById("app");a.innerHTML=`
        ...
        <header class="dash-topbar">
          <div class="dash-topbar-left">
            ...
            <div>
              ...
              <p class="dash-page-subtitle">${xe(m)}</p>
            </div>
          </div>
          <div class="dash-topbar-right">
            <div class="dash-user-menu" id="user-menu">
              ...
              <div class="dash-dropdown" id="user-dropdown" style="display: none;">
                ...
                ${b==="admin"?`<button class="dash-dropdown-item" id="menu-admin">${e("dash_user_menu_admin")}</button>`:""}
  ...
  `,_e(),A(),ue()}function ke(s){const a=document.getElementById("app");a.innerHTML=`
    <div class="dash-layout">
      <aside class="dash-sidebar" id="dash-sidebar">
        <div class="dash-sidebar-header">
          <a href="/dragonwebsite/" class="dash-logo">
  ...
  `,_e(),A(),ue()}function te(){if(!o||!o.parent)return null;const s=o.parent;return[s.firstName,s.lastName].filter(Boolean).join(" ")||null}function pe(s,a="swimmer"){return a==="coach"?{overview:e("dash_coach_tab_overview"),roster:e("dash_coach_tab_roster"),meets:e("dash_coach_tab_meets"),schedule:e("dash_coach_tab_schedule")}[s]||e("dash_coach_tab_overview"):{overview:e("dash_swimmer_tab_overview"),profile:e("dash_swimmer_tab_profile"),plans:e("dash_swimmer_tab_plans"),meets:e("dash_swimmer_tab_meets"),schedule:e("dash_swimmer_tab_schedule")}[s]||e("dash_swimmer_tab_overview")}function xe(s){return{overview:e("dash_swimmer_overview_sub"),profile:e("dash_swimmer_profile_sub"),plans:e("dash_swimmer_plans_sub"),meets:e("dash_swimmer_meets_sub"),schedule:e("dash_swimmer_schedule_sub")}[s]||""}function he(s,a="swimmer"){if(a==="coach")switch(s){case"overview":return de();case"roster":return De();case"meets":return ne();case"schedule":return ie();default:return de()}switch(s){case"overview":return Ce();case"profile":return Ae();case"plans":return Ne();case"meets":return ne();case"schedule":return ie();default:return""}}function ue(){const s=document.getElementById("sidebar-theme-icon");if(s){const a=document.documentElement.getAttribute("data-theme")==="dark";s.textContent=a?"☀️":"🌙"}}function ve(){const s=[];for(const a of k)if(a.swimmers)for(const n of a.swimmers)n.deleted||s.push({...n,parentName:fe(a)});return s}function fe(s){return s.parent&&[s.parent.firstName,s.parent.lastName].filter(Boolean).join(" ")||"—"}function Be(){const s=new Date;return s.setDate(s.getDate()-30),k.filter(a=>{var t,i;return(((i=(t=a.createdAt)==null?void 0:t.toDate)==null?void 0:i.call(t))||new Date(a.createdAt))>=s})}function de(){const s=ve(),a=Be(),n=w.filter(t=>t.status!=="Completed");return`
  ...
  `}function Ce(){const s=C.filter(t=>t.status!=="Completed").length,a=C.filter(t=>t.status==="Completed").length,n=w.filter(t=>t.status!=="Completed").length;return`
    <div class="dash-stats-row">
      <div class="dash-stat-card">
    ...
    <div class="dash-overview-grid">
      ...
      <div class="dash-panel">
        ...
        <div class="dash-panel-body">
          ${w.filter(t=>t.status!=="Completed").map(t=>Se(t)).join("")}
        </div>
      </div>
    </div>
  ...
  `}function Se(s){const a=s.status||"Open";return`
    <div class="dash-mini-card">
      <div class="dash-mini-top">
  ...
  `}function Le(){const s=new Date().getDay(),a=$(s),n=N.filter(t=>t.day===a);return n.length===0?`<p class="dash-empty">${e("dash_swimmer_rest_day")} (${a}). Rest day! 🎉</p>`:n.map(t=>`
    <div class="dash-mini-card">
      <div class="dash-mini-top">
  ...
  `).join("")}function Ae(){if(!o)return`<div class="dash-panel" style="text-align: center; padding: 3rem;">
  ...
  `}function ne(){const s=g==="coach";return`
  ...
  `}function ie(){const s=g==="coach";[0,1,2,3,4,5,6].map(n=>$(n));const a=[1,2,3,4,5,6,0];return`
    <div class="dash-section-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
    ...
    <div class="dash-schedule-grid">
      ${a.map(n=>{const t=$(n),i=N.filter(c=>c.day===t);return`
  ...
  `}function qe(s,a){const n=document.createElement("div");n.className="confirm-overlay",n.innerHTML=`

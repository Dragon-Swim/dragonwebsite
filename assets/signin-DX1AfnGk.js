import{i as O,t as s}from"./i18n-DIN1ScE8.js";import{a as Q,r as Z}from"./footer-Cg_RfItd.js";import{i as w,q as y,c as b,e as r,w as v,d as G,b as $,m as j,g as m,u as S,n as N,h as D,p as V,j as Y,t as J,s as K}from"./firebase-CorBctTj.js";O();Q();const X=new URLSearchParams(window.location.search);let g=X.get("mode")==="signup",C=!1,p=0;const ee=document.getElementById("app");function B(){ee.innerHTML=`
        ...
        <div class="signin-wrapper">
          <div class="signin-card">
            ...
            <div class="signin-header">
              ...
              <h1 class="signin-title">${g?s("signup_title"):s("signin_title")}</h1>
            ...
            <form class="signin-form" id="auth-form">
              ...
              `:""}
              <div class="form-group">
                <label class="form-label" for="auth-email">${g?s("signup_email"):s("signin_email")}</label>
                <input class="form-input" type="email" id="auth-email" placeholder="you@example.com" required ${p>=3?"disabled":""} />
              </div>
              <div class="form-group">
                <label class="form-label" for="auth-password">${g?s("signup_password"):s("signin_password")}</label>
            ...
            <div class="signin-toggle">
              ${g?`${s("signup_has_account")} <a href="#" id="toggle-auth">${s("signup_signin_link")}</a>`:`${s("signin_no_account")} <a href="#" id="toggle-auth">${s("signin_signup_link")}</a>`}
            </div>
            `}
  ...
  `,Z(),te()}function te(){var P,z,q,L,F,M,R;const _=document.getElementById("auth-error"),A=document.getElementById("submit-btn");function k(e){_&&(_.textContent=e,_.style.display="block")}document.querySelectorAll(".password-toggle").forEach(e=>{e.addEventListener("click",()=>{const o=document.getElementById(e.dataset.target),t=o.type==="password";o.type=t?"text":"password"})});function H(e){const o=document.getElementById("pw-strength"),t=document.getElementById("pw-bar-fill"),i=document.getElementById("pw-label");if(!o||!t||!i)return;if(!e){o.style.display="none";return}o.style.display="block";const a={length:e.length>=8,upper:/[A-Z]/.test(e),lower:/[a-z]/.test(e),digit:/[0-9]/.test(e),special:/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(e)};let n=0;for(const[l,E]of Object.entries(a)){const d=document.querySelector(`.password-strength-rule[data-rule="${l}"]`);if(!d)continue;const c=d.querySelector(".rule-icon");E?(c.textContent="✅",d.style.color="#16A34A",n++):(c.textContent="❌",d.style.color="")}const u=n/5*100;t.style.width=u+"%",n<=2?(t.style.background="#DC2626",i.textContent="Weak",i.style.color="#DC2626"):n<=4?(t.style.background="#F59E0B",i.textContent="Fair",i.style.color="#F59E0B"):(t.style.background="#16A34A",i.textContent="Strong",i.style.color="#16A34A")}(P=document.getElementById("auth-password"))==null||P.addEventListener("input",e=>{H(e.target.value)}),(z=document.getElementById("toggle-auth"))==null||z.addEventListener("click",e=>{var o;e.preventDefault(),g=!g,C=!1,p=0,(o=document.querySelector(".footer"))==null||o.remove(),B()}),(q=document.getElementById("auth-form"))==null||q.addEventListener("submit",async e=>{var i;e.preventDefault();const o=document.getElementById("auth-email").value,t=document.getElementById("auth-password").value;_.style.display="none",A.disabled=!0;try{if(g){const a=document.getElementById("auth-confirm").value;if(t!==a)throw new Error("Passwords do not match.");const n={length:t.length>=8,upper:/[A-Z]/.test(t),lower:/[a-z]/.test(t),digit:/[0-9]/.test(t),special:/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(t)};if(Object.values(n).filter(Boolean).length<5)throw new Error("Password must be at least 8 characters and include uppercase, lowercase, a number, and a special character.");const l=o.toLowerCase().trim(),E=await w(y(b(r,"coaches"),v("email","==",l))),d=await w(y(b(r,"families"),v("email","==",l)));let c=null,h=null,f=null;if(E.empty?d.empty||(h=d.docs[0],f="families",c="swimmer"):(h=E.docs[0],f="coaches",c=h.data().role||"coach"),!h)throw new Error(s("signup_unauthorized"));const x=(await G($,o,t)).user;if(await j(m(r,"users",x.uid),{email:o,role:c,createdAt:new Date}),await S(m(r,f,h.id),{status:f==="coaches"?"active":"registered",registeredUid:x.uid}),c==="coach"||c==="admin")window.location.href="/dashboard.html";else{const W=y(b(r,"registrations"),v("parentEmails","array-contains",l)),T=await w(W);if(T.empty)window.location.href="/registration.html";else{const U=T.docs[0],I=U.data().editors||[];I.includes(x.uid)||(I.push(x.uid),await S(m(r,"registrations",U.id),{editors:I})),window.location.href="/dashboard.html"}}}else try{const a=await N($,o,t),n=o.toLowerCase().trim(),u=await D(m(r,"users",a.user.uid)),l=u.exists()?u.data().role:null;if(l==="removed"){const d=await w(y(b(r,"coaches"),v("email","==",n)));if(!d.empty){const c=d.docs[0].data().role||"coach";await S(m(r,"users",a.user.uid),{role:c}),window.location.href="/dashboard.html";return}throw new Error("Your coach access has been revoked. Please contact admin@dragonswim.com.")}if(l==="coach"||l==="admin"){window.location.href="/dashboard.html";return}if((await D(m(r,"registrations",a.user.uid))).exists())window.location.href="/dashboard.html";else{const d=y(b(r,"registrations"),v("parentEmails","array-contains",n));(await w(d)).empty?window.location.href="/registration.html":window.location.href="/dashboard.html"}}catch(a){throw a.code==="auth/wrong-password"||a.code==="auth/invalid-credential"?new Error("Wrong password. Please try again."):a}}catch(a){console.error(a),g||p++,p>=3?(k("Too many failed attempts. Please reset your password below."),(i=document.querySelector(".footer"))==null||i.remove(),B()):(k(a.message||"Authentication failed"),A.disabled=!1)}}),(L=document.getElementById("google-signin"))==null||L.addEventListener("click",async()=>{_.style.display="none";try{const e=await V($,Y),t=e.user.email.toLowerCase().trim(),i=await w(y(b(r,"coaches"),v("email","==",t))),a=await w(y(b(r,"families"),v("email","==",t)));let n=null,u=null,l=null;if(i.empty?a.empty||(u=a.docs[0],l="families",n="swimmer"):(u=i.docs[0],l="coaches",n=u.data().role||"coach"),!u)throw await J($),new Error(s("signup_unauthorized_google"));if(u.data().status==="pending"&&await S(m(r,l,u.id),{status:l==="coaches"?"active":"registered",registeredUid:e.user.uid}),await j(m(r,"users",e.user.uid),{username:e.user.displayName||"Google User",email:e.user.email,role:n,lastLoginAt:new Date},{merge:!0}),n==="coach"||n==="admin")window.location.href="/dashboard.html";else if((await D(m(r,"registrations",e.user.uid))).exists())window.location.href="/dashboard.html";else{const d=y(b(r,"registrations"),v("parentEmails","array-contains",t)),c=await w(d);if(c.empty)window.location.href="/registration.html";else{const h=c.docs[0],f=h.data().editors||[];f.includes(e.user.uid)||(f.push(e.user.uid),await S(m(r,"registrations",h.id),{editors:f})),window.location.href="/dashboard.html"}}}catch(e){console.error(e),e.code==="auth/popup-closed-by-user"?k("Sign-in popup closed before completion."):e.code==="auth/cancelled-popup-request"||k(e.message||"Google sign in failed")}}),(F=document.getElementById("forgot-link"))==null||F.addEventListener("click",e=>{var o;e.preventDefault(),C=!0,p=0,(o=document.querySelector(".footer"))==null||o.remove(),B()}),(M=document.getElementById("forgot-form"))==null||M.addEventListener("submit",async e=>{e.preventDefault();const o=document.getElementById("forgot-email").value,t=document.getElementById("auth-error"),i=document.getElementById("forgot-success"),a=document.getElementById("forgot-submit-btn");t&&(t.style.display="none"),i&&(i.style.display="none"),a&&(a.disabled=!0);try{await K($,o),i&&(i.textContent=s("signin_forgot_success"),i.style.display="block")}catch(n){console.error("Password reset error:",n),t&&(t.textContent=n.code==="auth/user-not-found"?s("signin_forgot_error"):n.message||s("signin_forgot_error"),t.style.display="block")}finally{a&&(a.disabled=!1)}}),(R=document.getElementById("forgot-back"))==null||R.addEventListener("click",e=>{var o;e.preventDefault(),C=!1,p=0,(o=document.querySelector(".footer"))==null||o.remove(),B()})}B();

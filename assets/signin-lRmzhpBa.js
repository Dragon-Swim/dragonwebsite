import{i as k,t as e}from"./i18n-cfzYsXIq.js";import{a as x,r as I}from"./footer-9Zbu_KFG.js";import{d as S,b as u,l as E,g as _,e as $,m as P,n as D,i as A,s as C}from"./firebase-8Bk_msJm.js";k();x();const q=new URLSearchParams(window.location.search);let i=q.get("mode")==="signup",m=!1,a=0;const L=document.getElementById("app");function d(){L.innerHTML=`
  ...
  `,I(),z()}function z(){var f,h,b,w,y,v;const l=document.getElementById("auth-error"),p=document.getElementById("submit-btn");function g(t){l&&(l.textContent=t,l.style.display="block")}document.querySelectorAll(".password-toggle").forEach(t=>{t.addEventListener("click",()=>{const s=document.getElementById(t.dataset.target),n=s.type==="password";s.type=n?"text":"password"})}),(f=document.getElementById("toggle-auth"))==null||f.addEventListener("click",t=>{var s;t.preventDefault(),i=!i,m=!1,a=0,(s=document.querySelector(".footer"))==null||s.remove(),d()}),(h=document.getElementById("auth-form"))==null||h.addEventListener("submit",async t=>{var r;t.preventDefault();const s=document.getElementById("auth-email").value,n=document.getElementById("auth-password").value;l.style.display="none",p.disabled=!0;try{if(i){const o=document.getElementById("auth-confirm").value;if(n!==o)throw new Error("Passwords do not match.");const B=(await S(u,s,n)).user;await E(_($,"users",B.uid),{email:s,role:"swimmer",createdAt:new Date}),window.location.href="/dragonwebsite/registration.html"}else try{await P(u,s,n),window.location.href="/dragonwebsite/dashboard.html"}catch(o){throw o.code==="auth/wrong-password"||o.code==="auth/invalid-credential"?new Error("Wrong password. Please try again."):o}}catch(o){console.error(o),i||a++,a>=3?(g("Too many failed attempts. Please reset your password below."),(r=document.querySelector(".footer"))==null||r.remove(),d()):(g(o.message||"Authentication failed"),p.disabled=!1)}}),(b=document.getElementById("google-signin"))==null||b.addEventListener("click",async()=>{l.style.display="none";try{const t=await D(u,A);await E(_($,"users",t.user.uid),{username:t.user.displayName||"Google User",email:t.user.email,role:"swimmer",lastLoginAt:new Date},{merge:!0}),window.location.href="/dragonwebsite/dashboard.html"}catch(t){console.error(t),t.code==="auth/popup-closed-by-user"?g("Sign-in popup closed before completion."):t.code==="auth/cancelled-popup-request"||g(t.message||"Google sign in failed")}}),(w=document.getElementById("forgot-link"))==null||w.addEventListener("click",t=>{var s;t.preventDefault(),m=!0,a=0,(s=document.querySelector(".footer"))==null||s.remove(),d()}),(y=document.getElementById("forgot-form"))==null||y.addEventListener("submit",async t=>{t.preventDefault();const s=document.getElementById("forgot-email").value,n=document.getElementById("auth-error"),r=document.getElementById("forgot-success"),o=document.getElementById("forgot-submit-btn");n&&(n.style.display="none"),r&&(r.style.display="none"),o&&(o.disabled=!0);try{await C(u,s),r&&(r.textContent=e("signin_forgot_success"),r.style.display="block")}catch(c){console.error("Password reset error:",c),n&&(n.textContent=c.code==="auth/user-not-found"?e("signin_forgot_error"):c.message||e("signin_forgot_error"),n.style.display="block")}finally{o&&(o.disabled=!1)}}),(v=document.getElementById("forgot-back"))==null||v.addEventListener("click",t=>{var s;t.preventDefault(),m=!1,a=0,(s=document.querySelector(".footer"))==null||s.remove(),d()})}d();
